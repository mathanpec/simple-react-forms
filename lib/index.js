	  ...
	  _createClass(Field, [{
	    ...
	    value: function checkForValidation(value) {
	      ...
	      return validators.reduce(function () {
	        var ref = _asyncToGenerator(regeneratorRuntime.mark(function _callee(currentState, validator) {
	          return regeneratorRuntime.wrap(function _callee$(_context) {
	            while (1) {
	              switch (_context.prev = _context.next) {
	                case 0:
	                  _context.next = 2;
	                  return currentState.then(function (state) {
	                    if (state.valid) {
	    ...
	    value: function setFieldState(value) {
	      ...
	      return this.checkForValidation(value, validators).then(function (validationState) {
	        var valid = validationState.valid;
	        var error = validationState.error;

	        _this2.setState({ value: value, valid: valid, error: error, touched: touched, hasValidationFinished: true });
	      });
	    }
	  }, {
	    ...
	    value: function onChangeHandler(event) {
	      ...
	      event.persist();
	      this.setFieldState(this.props.valueAccessor ? this.props.valueAccessor(event) : this.defaultValueAccessors(event), true).then(function (_) {
	        _this3.props.onChange && _this3.props.onChange(event);
	      });
	    }
	  }, {
